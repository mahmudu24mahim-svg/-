...
def sms_start(m):
    ...
    bot.send_message(m.chat.id,"📱 Enter number",reply_markup=back_kb())
...
def sms_flow(m):
    ...
    if state["step"] == "number":
        ...
        state["number"] = m.text
        state["step"] = "limit"
        bot.send_message(m.chat.id,"✍ Enter Attack limit")
        return
    ...
    if state["step"] == "limit":
        ...
        bot.send_message(m.chat.id,f"⏳ Sending {send_count} Attack")
        ...
        bot.send_message(m.chat.id,f"✅ Done ({success} Attack)",reply_markup=main_menu(uid))
...
def buy(m):
    bot.send_message(m.chat.id,f"Contact admin:\n{ADMIN_CONTACTS}")
...
def admin_panel(m):
    ...
    bot.send_message(
        ...
        f"💰 Total Balance: {total_balance} Balance",
        reply_markup=kb
    )
